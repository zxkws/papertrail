...
- 位图字体只能在服务端已装的系统字体里挑最接近的：原图用的字体服务器没有时，匹配分数会明显偏低，但仍会挑一个形状最接近的顶上。
- 落笔前会检查字体能不能渲染这段文字，缺字（典型是拉丁字体遇到中文）会自动换成画得出的字体，并在响应的 `used` 里记 `font_fallback_from`。
- 纹理背景擦除靠 inpainting 推测，纹理越规则越容易看出补丁。
