...
export function deriveElements(
  ...
  for (const l of layouts)
    for (const e of l.elements)
      state.set(e.id, {
        ...
        text: e.text,
        bbox: [...e.bbox],
        style: { ...fallback, font_size_pt: e.font_size, color: e.color },
        kind: "native",
        deleted: false,
        changed: false,
  ...
  for (const op of [...ops].sort((a, b) => a.seq - b.seq)) {
    ...
    if (op.type === "add_text") {
      state.set(op.created_element_id!, {
        id: op.created_element_id!,
    ...
    const e = state.get(op.target_element_id!);
    if (!e || e.deleted) continue;
    e.changed = true;
    if (op.type === "delete_text") e.deleted = true;
    if (op.type === "move_text" && op.bbox) e.bbox = [...op.bbox];
    if (op.type === "replace_text") {
      if (op.payload?.text !== undefined) e.text = op.payload.text;
      if (op.bbox) e.bbox = [...op.bbox];
      if (op.style) e.style = { ...op.style };
    }
  }
  return [...state.values()];
}

export const covers = (ops: Operation[]) =>
  ops
    .filter((o) => o.type === "cover_region" && o.bbox)
    .map((o) => ({
      id: o.id,
      page_index: o.page_index,
      bbox: o.bbox!,
      color: o.payload?.cover_color ?? "#FFFFFF",
    }));
