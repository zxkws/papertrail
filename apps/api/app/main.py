...
def missing(exc: Exception):
    raise HTTPException(404, "resource not found") from exc


@app.get("/api/v1/health")
def health(): return {"status": "ok", "engine": fitz.VersionBind}
...
async def upload(file: UploadFile = File(...)):
    ...
    return {"document_id": meta["id"], "status": "READY", "upload_sha256": meta["source_sha256"], "page_count": pages}


@app.get("/api/v1/documents/{document_id}")
def get_document(document_id: str):
    try: return storage.document(document_id)
    except KeyError as exc: missing(exc)


@app.get("/api/v1/documents/{document_id}/file")
def source_file(document_id: str):
    try: meta = storage.document(document_id)
    except KeyError as exc: missing(exc)
    return FileResponse(meta["source_path"], media_type="application/pdf", filename=meta["filename"])
...
def create_draft(document_id: str):
    try: return storage.create_draft(document_id)
    except KeyError as exc: missing(exc)


@app.get("/api/v1/drafts/{draft_id}")
def get_draft(draft_id: str):
    try: return storage.draft(draft_id)
    except KeyError as exc: missing(exc)
...
def draft_native(draft_value: dict) -> list[NativeElement]:
    ...
    for page in range(meta["page_count"]):
        result.extend(NativeElement.model_validate(e) for e in parse_layout(meta["source_path"], meta["source_sha256"], page)["elements"])
    return result


@app.put("/api/v1/drafts/{draft_id}/operations")
def save_operations(draft_id: str, body: SaveOperations):
    try: value = storage.draft(draft_id)
    except KeyError as exc: missing(exc)
    if body.expected_revision != value["revision"]:
        raise HTTPException(409, detail={"code": "DRAFT_REVISION_CONFLICT", "latest_revision": value["revision"]})
    try: canonical = canonical_reduce(draft_native(value), body.operations)
    except ReducerError as exc: raise HTTPException(422, str(exc)) from exc
    value.update(revision=value["revision"] + 1, operations=[op.model_dump() for op in body.operations], canonical_hash=canonical["canonical_hash"])
    storage.save_draft(value)
    return {"revision": value["revision"], "operation_set_hash": canonical["canonical_hash"]}


@app.post("/api/v1/drafts/{draft_id}/exports", status_code=201)
def create_export(draft_id: str):
    try: value = storage.draft(draft_id); meta = storage.document(value["document_id"])
    except KeyError as exc: missing(exc)
    ...
    version = storage.publish_export(meta["id"], temp, canonical["canonical_hash"], value["revision"])
    return {"version_id": version["id"], "canonical_hash": canonical["canonical_hash"], "sha256": version["sha256"], "status": "READY"}


@app.get("/api/v1/versions/{version_id}/download")
def download(version_id: str):
    try: value = storage.version(version_id)
    except KeyError as exc: missing(exc)
    return FileResponse(value["path"], media_type="application/pdf", filename=f"edited-{version_id}.pdf")
