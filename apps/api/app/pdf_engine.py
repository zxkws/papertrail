            ...
            for line in block.get("lines", []):
                for span in line.get("spans", []):
                    ...
                    element = NativeElement(
                        ...
                        page_index=page_index, text=text, bbox=tuple(round(float(x), 4) for x in span["bbox"]),
                        font_name=span.get("font", "unknown"), font_size=span.get("size", 11),
                        color=color_hex(span.get("color", 0)), flags=span.get("flags", 0), editability=editable,
                    )
                    elements.append(element.model_dump()); ordinal += 1; chars += len(text)
        rect = page.rect
        return {
            "schema_version": "v1", "page_index": page_index,
            "width_pt": rect.width, "height_pt": rect.height,
            "crop_box": list(page.cropbox), "media_box": list(page.mediabox),
            "rotation": page.rotation if page.rotation in (0, 90, 180, 270) else 0,
            "scan_likelihood": 1.0 if chars == 0 else 0.0, "elements": elements,
        }
    finally:
        doc.close()
    ...
    try:
        ...
        for phase in ("redactions", "covers", "inserts"):
            for item in canonical[phase]:
                by_page.setdefault(item["page_index"], {"redactions": [], "covers": [], "inserts": []})[phase].append(item)
        for page_index, phases in by_page.items():
            ...
            for item in phases["covers"]:
                page.draw_rect(fitz.Rect(item["bbox"]), color=_rgb(item["color"]), fill=_rgb(item["color"]), overlay=True)
            ...
            for item in phases["inserts"]:
                ...
                while font_size >= 4 and result < 0:
                    result = page.insert_textbox(rect, item["text"], fontsize=font_size, fontname="helv", color=_rgb(style.get("color", "#111111")), rotate=rotation, align={"left": 0, "center": 1, "right": 2}.get(style.get("align"), 0), overlay=True)
                    if result < 0:
                        font_size -= 0.5
                if result < 0:
                    raise ValueError(f"TEXT_OVERFLOW:{item['target_id']}")
        tmp = tempfile.NamedTemporaryFile(prefix="pdf-export-", suffix=".pdf", delete=False)
        tmp.close()
        path = Path(tmp.name)
